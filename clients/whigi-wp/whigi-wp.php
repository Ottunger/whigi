...
if(!class_exists('WHIGI')) {
	class WHIGI {
		...
		private $settings = array(
			...
			'whigi_whigi_data' => 'profile/first_name//profile/last_name//profile/email//profile/address',
			'whigi_whigi_time' => '1',
...
-----END RSA PRIVATE KEY-----',
		...
		function init() {
			...
			add_action('customize_register', array($this, 'whigi_login_form_in_reg'));
			add_action('login_form', array($this, 'whigi_sole_login'));
			add_action('register_form', array($this, 'whigi_sole_login'));
			add_action('load-user-new.php', array($this, 'whigi_no_new'));
		...
		//Clear what WP has added
		function whigi_sole_login() {
			$cnt = ob_get_contents();
			$cnt = preg_replace("/<form name=/", "<form style=\"display:none;\" name=", $cnt);
			while(@ob_get_clean());
			echo $cnt;
		}
		...
		function whigi_login_form_in_reg($manager){
			$manager->add_section('whigi',$this->whigi_login_form(array()));
		}
