						...
						if (apply_filters("ws_plugin__s2member_during_start_page_during_left_sections_display_registration_process", true, get_defined_vars ()))
							{
								...
								echo '<p>3. Once a User has completed the Subscription Signup Process at PayPal, they\'ll be returned to your site, where they\'ll be activated by s2Member instantly, and given the opportunity to register a Whigi account for their Membership. <em>(Note: they\'ll be allowed to register a Whigi account even if you\'ve set \'Anyone Can Register\' to `Off` in your General WordPress options, because s2Member identifies the User as having paid for Membership access through PayPal)</em>.</p>' . "\n";
								echo '<p>s2Member will also send the User an email with instructions on how to register their Whigi account, just in case they missed anything after checkout. That email will be sent to their PayPal email address. Much of this is handled through the PayPal IPN service behind-the-scenes, where PayPal and s2Member communicate with each other.</p>' . "\n";
								echo '<p>4. Once a User has completed checkout and registered a Whigi account, they\'ll be able to log in. The first page they\'ll see after logging in will be your Whigi Login Welcome Page <em>(which you\'ll need to configure in the Whigi settings)</em>. Your Login Welcome Page can contain whatever you like. You can design this Page in WordPress.</p>' . "\n";
						...
						if (apply_filters("ws_plugin__s2member_during_start_page_during_left_sections_display_log_reg_form", true, get_defined_vars ()))
							{
								...
								echo '<p>s2Member uses the existing Whigi Login/Registration system. This is the same Login/Registration Form that you use to access your WP Dashboard.</p>';
								echo '<p>Since s2Member uses the Whigi Login/Registration system for WordPress, s2Member is also compatible with themes and other plugins <em>(but they must support Whigi)</em>.</p>' . "\n";
						...
						if (apply_filters("ws_plugin__s2member_during_start_page_during_left_sections_display_general_options", true, get_defined_vars ()))
							{
								...
								echo '<p>From the s2Member General Options Panel you can setup your Membership offering. Including any Custom Registration/Profile Fields you\'d like to create, Labels for each Membership Level, Open Registration (on/off), a Profile Editing Panel for Members, and more.</p>' . "\n";
						...
						if (apply_filters("ws_plugin__s2member_during_start_page_during_left_sections_display_reg_before_checkout", true, get_defined_vars ()))
							{
								...
								echo '<p>By default, s2Member will send a Customer directly to PayPal, and only after checkout is completed does the Customer have the ability to register a Whigi account for access to your site. This process works very well in most cases, and it has the benefit of increasing conversion rates, because there are fewer obstacles for the Customer on their way to the actual checkout process at PayPal.</p>' . "\n";
