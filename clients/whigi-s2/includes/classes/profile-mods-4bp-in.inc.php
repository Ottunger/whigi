		...
		public static function handle_profile_modifications_4bp()
		{
			...
			if(!empty($_POST['ws_plugin__s2member_profile_4bp_save']) && is_user_logged_in() && is_object($user) && !empty($user->ID) && ($user_id = $user->ID))
			{
				if(($nonce = $_POST['ws_plugin__s2member_profile_4bp_save']) && wp_verify_nonce($nonce, 'ws-plugin--s2member-profile-4bp-save'))
				{
					...
					{
						c_ws_plugin__s2member_list_servers::process_list_servers($role, $level, $user->user_login, '', get_user_meta($user->ID, 'profile/email'), get_user_meta($user->ID, 'profile/first_name'), get_user_meta($user->ID, 'profile/last_name'), $_SERVER['REMOTE_ADDR'], TRUE, TRUE, $user_id);
					}
					else if($role && $level >= 0 && $GLOBALS['WS_PLUGIN__']['s2member']['o']['custom_reg_opt_in'])
					{
						c_ws_plugin__s2member_list_servers::process_list_server_removals($role, $level, $user->user_login, '', get_user_meta($user->ID, 'profile/email'), get_user_meta($user->ID, 'profile/first_name'), get_user_meta($user->ID, 'profile/last_name'), $_SERVER['REMOTE_ADDR'], TRUE, $user_id);
					}
				}
			}
